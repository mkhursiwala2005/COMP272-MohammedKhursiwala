...
class myHashMap<K,V> {
    ...
    public V remove(K key) {
        ...
        HashNode<K,V> currNode = bucket.get(i);
        HashNode<K,V> prevNode = null;
        V val = get(key);
        while (currNode != null) {
            if (currNode.key.equals(key) && currNode.value.equals(val)) {
                // previous node is null then the node is the head
                if (prevNode == null) {
                    bucket.set(i, currNode.next);
                } else {
