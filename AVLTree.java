...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        // if a particular node doesn't exist in the tree, return null
        if (node == null) {
            return null;
        }
        ...
        // if the node we want to delete exists,
        // that is value of the node is the same as parameter value
        if (node.value == value) {
            // 1st case: leaf node
            // check if the node's leftChild and rightChild are null, then simply return null
            if (node.leftChild == null && node.rightChild == null) {
                return null;
            }
            // 2nd case: node has only one child
            // check if node leftChild is null then return node rightChild or vice versa
            ...
            // 3rd case: node has two children then carry out an inorder traversal
            else {
                Node n = minValueNode(node.rightChild);
                node.value = n.value;
                node.rightChild = deleteElement(n.value, node.rightChild);
            }
        ...
        // depending on the balance factor, decide whether to update the tree
        if (bf > 1) {
            // left heavy
            if (getBalanceFactor(node.leftChild) > 0) {
                return RRRotation(node);
            } else {
                node.leftChild = LLRotation(node);
                return RRRotation(node);
            }
            ...
            if (getBalanceFactor(node.rightChild) <= 0) {
               return LLRotation(node);
                ...
                node.rightChild = RRRotation(node);
                return LLRotation(node);
            }
        }
